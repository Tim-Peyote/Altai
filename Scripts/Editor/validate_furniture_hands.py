...
w,pc=context();p=unreal.GameplayStatics.get_player_pawn(w,0);h=pc.hands;h.release();p.set_first_person(True);p.crouch()
...
def finish(error=''):
 ...
 Path(unreal.Paths.project_saved_dir(),'furniture_hand_validation.json').write_text(json.dumps({'passed':not error and all(r.get('grab') and r.get('dragged',0)>3 and r.get('max_wrist_error',999)<9 and r.get('still_held') and r.get('look_restored') and r.get('tag_cleared') and r.get('carry_speed')==1 for r in report),'error':error,'cases':report},indent=2));unreal.log('FURNITURE_HAND_TEST_DONE')
def tick(dt):
 global idx,phase,began,row,samples
 try:
  now=time.monotonic();a=props[idx]
  if phase=='prepare':
   h.release();p.character_movement.stop_movement_immediately();grip=a.part.get_socket_location('Grip_One');p.set_actor_location(unreal.Vector(grip.x+48,grip.y+12,197),False,True);pc.set_control_rotation(unreal.Rotator(pitch=-20,yaw=180,roll=0));p.set_actor_rotation(unreal.Rotator(pitch=0,yaw=180,roll=0),True)
   p.crouch()
  ...
  if phase=='aim' and now-began>1:
   row['pawn']=str(p.get_actor_location());row['shoulder']=str(p.mesh.get_socket_location('upperarm_r'));row['target']=str(a.part.get_socket_location('Grip_One'));row['shoulder_distance']=(p.mesh.get_socket_location('upperarm_r')-a.part.get_socket_location('Grip_One')).length();row['grab']=h.try_grab(True);row['hint']=h.hint;row['held']=h.held.get_owner().get_actor_label() if h.held else None
  ...
  if phase=='hold':
   if not h.held:row['lost']=h.hint;row['grab']=False;phase='done'
    ...
    row['grip_error']=(p.mesh.get_socket_location('hand_r')-a.part.get_socket_location('Grip_One')).length();h.drag_interaction(9 if a.sliding else 12);phase='drag';began=now
   return
  if phase=='drag':
   samples.append((p.mesh.get_socket_location('hand_r')-a.part.get_socket_location('Grip_One')).length())
