"""Slow visual review of the reverse mantle; automatic time-scale cleanup."""
import unreal,time
...
p.set_actor_location(unreal.Vector(center.x,center.y,center.z+ext.z+98),False,True);p.character_movement.stop_movement_immediately();p.set_actor_rotation(unreal.Rotator(yaw=0),True);pc.set_control_rotation(unreal.Rotator(pitch=-15,yaw=0));p.set_first_person(False);pc.show_lab_hud=True
...
def tick(dt):
 ...
 if not started:
  started=True;t.try_descend();unreal.GameplayStatics.set_global_time_dilation(w,.15);pc.set_control_rotation(unreal.Rotator(pitch=-12,yaw=95,roll=0));unreal.WidgetLibrary.set_input_mode_game_only(pc,True)
 elif t.climbing and t.progress>.4 and not paused_at:
  paused_at=time.monotonic();unreal.GameplayStatics.set_global_time_dilation(w,.001)
 elif paused_at and time.monotonic()-paused_at>40 and t.climbing:
