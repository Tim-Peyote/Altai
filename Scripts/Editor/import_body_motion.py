"""Bake retargeted CMU motion into native Unreal animation assets."""
import unreal,json
...
for label in ['GetUpProne','GetUpSupine']:
 data=json.loads((root/'SourceArt/Motion/CMU140'/(label+'.json')).read_text());name='A_'+label
 ...
 controller.open_bracket('Retarget CMU recovery',False);controller.remove_all_bone_tracks(False);controller.set_frame_rate(unreal.FrameRate(30,1),False);controller.set_number_of_frames(unreal.FrameNumber(len(data['frames'])-1),False)
