...
def finish(error=''):
 ...
 states=sorted(set(r['state'] for r in rows));checks={'physical_fall':any('FALLING' in s for s in states),'recovery_started':any('GETTING_UP' in s for s in states),'recovery_finished':body.recoveries>0,'movement_restored':not pc.is_move_input_ignored(),'finite':all(all(math.isfinite(v) for v in xyz) for r in rows for xyz in r['bones'].values())}
...
def tick(dt):
 ...
 try:
  ...
  if elapsed>18 or (elapsed>4 and body.recoveries>0):finish()
