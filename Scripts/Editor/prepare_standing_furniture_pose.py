...
w,pc=context();p=unreal.GameplayStatics.get_player_pawn(w,0);h=pc.hands;h.release();p.set_first_person(True);p.un_crouch();pc.set_editor_property('show_lab_hud',True)
...
def tick(dt):
 ...
 try:
  ...
  if phase==0 and t>.8:
   p.character_movement.stop_movement_immediately();p.set_actor_location(unreal.Vector(grip.x+48,grip.y+12,247),False,True);pc.set_control_rotation(unreal.Rotator(pitch=-10,yaw=180,roll=0));phase=1;start=time.monotonic()
  elif phase==1:
